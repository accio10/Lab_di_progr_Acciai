...
TEST_F(TesterUser,
       transferUnvalidUser) {//eseguo test per verificare che il versamento non venga eseguito se supera una certa cifra
    tested->Operation(2000, TEST, accountnull);
    ASSERT_EQ(0, tested->getAccount()->getBalance());
}

TEST_F(TesterUser,
       checkactiveaccount) {//eseguo un test per verificare che l'attivazione dell' account sia stata eseguita
